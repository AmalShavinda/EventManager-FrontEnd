...
const DeactiveUsers = () => {
    const { data, loading, error, reFetch } = useFetch('http://localhost:8800/api/user/deactiveUsers');
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [newUser, setNewUser] = useState({ username: '', email: '', password: '', isAdmin: '' });
    ...
    const handleActivate = async (id) => {
        try {
            await axios.put(`http://localhost:8800/api/user/activate/${id}`, { status: "Active" });
            reFetch();
        } catch (error) {
    ...
    const handleAddUser = async () => {
        try {
            ...
            setNewUser({ username: '', email: '', password: '', isAdmin: '' });
        } catch (error) {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold">Deactive Users</h2>
                <button
